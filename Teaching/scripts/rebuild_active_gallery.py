...
for path in (PROJECT_ROOT, DINO_ROOT, CLASSIFIER_ROOT):
...
def embed_teaching_images(teaching_dir: Path, embedder) -> dict[tuple[str, str], list[np.ndarray]]:
    ...
    for class_name, object_name, image_path in store.iter_labeled_images():
        ...
        grouped[(class_name, object_name)].append(normalize(embedding))
    return grouped
...
def build_gallery(
    *,
    ...
    meta = {
        ...
        "taught_labels": [f"{class_name}/{object_name}" for class_name, object_name in sorted(taught)],
    }
    np.savez_compressed(
        output_path,
...
def update_config_gallery(config_path: Path, gallery_path: Path) -> None:
    import yaml
...
def main() -> None:
    ...
    if args.update_config:
        update_config_gallery(PROJECT_ROOT / "config.yaml", output_path)
